...
def extract_cctv_timestamp(frame):
    ...
    , roithresh = cv2.threshold(roi_gray, 180, 255, cv2.THRESH_BINARY)
...
def draw_label(frame, text1, text2, x1, y1, x2, y2, color):
    ...
    thickness  = 2
    padding    = 4
    (w1, h1),  = cv2.getTextSize(text1, font, fontscale, thickness)
    (w2, h2),  = cv2.getTextSize(text2, font, fontscale, thickness)
...
class ViolationVideoTracker:
    def init(self, track_id, frame_size, output_dir):
...
clip_model, , clippreprocess = open_clip.create_model_and_transforms(
